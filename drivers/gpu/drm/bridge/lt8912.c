...
//{ 1024, 768, 1344, 806, 160, 136, 6, 29 };  // 1024x768 Timing                                                                             
//{ 1024, 600, 1324, 628, 160, 116, 3, 24 };   // 1024x600 Timing  
{ 1280, 800, 1447, 823, 80, 17, 3, 10 };	 // 1280x800 Timing 
...
static void lt8912_init(struct lt8912 *lt)
{
	...
	dev_info(lt->dev, DRM_MODE_FMT "alex- \n", DRM_MODE_ARG(mode));
...
//    Core_PLL_Ratio.Temp32  = (hactive-1)*4096/(LVDS_Panel_Timing[H_act] - 1);
	regmap_write(lt->regmap[0], 0x8f, Core_PLL_Ratio.Temp8[0]) ;
	regmap_write(lt->regmap[0], 0x90, Core_PLL_Ratio.Temp8[1]) ;
//	regmap_write(lt->regmap[0], 0x8f, /* Core_PLL_Ratio.Temp8[0]*/0x01) ;
//	regmap_write(lt->regmap[0], 0x90, /* Core_PLL_Ratio.Temp8[1]*/0x14) ;

	
//    Core_PLL_Ratio.Temp32  = (vactive-1)*4096/(LVDS_Panel_Timing[V_act] - 1);


	regmap_write(lt->regmap[0], 0x91, Core_PLL_Ratio.Temp8[0]) ;
	regmap_write(lt->regmap[0], 0x92, Core_PLL_Ratio.Temp8[1]) ;

//	regmap_write(lt->regmap[0], 0x91, /* Core_PLL_Ratio.Temp8[0]*/0x35) ;
//	regmap_write(lt->regmap[0], 0x92, /* Core_PLL_Ratio.Temp8[1]*/0x13) ;
...
static int lt8912_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	printk(KERN_NOTICE"ALEX 8912 TEST end\n");
