...
struct ov5640 {
	...
	int gate_gpio,pwn_gpio, rst_gpio;
};
...
static int ov5640_gate_control(struct ov5640 *sensor, int enable)
{
	//enable DOVDD AVDD and DVDD regulator
	gpio_set_value(sensor->gate_gpio,enable);
	msleep(5);
}
			...
			const struct i2c_device_id *id)
{
	...
	if (!gpio_is_valid(sensor->gate_gpio))
		dev_warn(dev, "No sensor gate pin available");
	
	else {
		retval = devm_gpio_request_one(dev, sensor->gate_gpio,
				GPIOF_OUT_INIT_LOW, "ov5640_mipi_gate");
		if (retval < 0) {
